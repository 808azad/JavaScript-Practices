...
function observationSummary(data) {
  for (var i = 0; i < data.results.length; i++){
      console.log(`ID-${data.results[i].id} "${data.results[i].species_guess}" (Date: ${data.results[i].observed_on})`);
  }
}
...
function observationsByGeoPrivacy(data, geoPrivacy) {
   ...
   for(const observation of data.results){
    if ((observation.geoprivacy && observation.geoprivacy.toLowerCase() === geoPrivacy?.toLowerCase()) ||
...
function extractTimeZones(data) {
  var arrOfTimeZones = [];
  for(var i = 0; i < data.results.length; i++){
    if (data.results[i].created_time_zone && !arrOfTimeZones.includes(data.results[i].created_time_zone)) {
      arrOfTimeZones.push(data.results[i].created_time_zone);
    }
    if (data.results[i].observed_time_zone && !arrOfTimeZones.includes(data.results[i].observed_time_zone)) {
